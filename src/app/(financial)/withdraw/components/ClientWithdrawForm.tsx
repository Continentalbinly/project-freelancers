...
export default function ClientWithdrawForm({ user, profile }: any) {
  ...
  const feePercent = 0.005; // 0.5% fee for clients
  ...
  const validateAmount = (): boolean => {
    ...
    if (amt > credit) {
      setError(t("withdraw.errors.insufficientCredit") || "Insufficient credit");
      return false;
    }
    setError("");
    return true;
  ...
  const handleWithdrawAll = () => {
    setAmount(formatCurrency(String(credit)));
  };
    ...
    <>
      ...
      >
        ...
        <p className="text-sm text-text-secondary">
          {t("withdraw.client.description") || "Withdraw your accumulated credits to your bank account."}
        </p>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="backdrop-blur-xl rounded-2xl p-6 w-full max-w-sm shadow-xl space-y-3 bg-background border border-border">
            ...
            <div className="text-sm text-text-secondary space-y-1">
              <p>
                <strong>{t("withdraw.confirm.from")}:</strong> {t("common.credit")}
              </p>
