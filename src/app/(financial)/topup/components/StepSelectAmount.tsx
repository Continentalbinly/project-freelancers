...
import { Loader2, CreditCard, Sparkles, Shield } from "lucide-react";
import type { TopupSession, TopupPackage, UpdateSessionFunction } from "@/types/topup";
...
export default function StepSelectAmount({
  ...
  t,
  userId,
}: StepSelectAmountProps) {
  const [loadingPkg, setLoadingPkg] = useState<number | null>(null);

  const choosePkg = async (pkg: TopupPackage) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-linear-to-br from-primary/20 dark:from-primary/30 to-secondary/20 dark:to-secondary/30 mb-4">
        ...
        {packages.map((pkg) => (
          <button
            ...
            className={`relative group p-6 rounded-2xl border-2 transition-all duration-300
              ...
              ${
                pkg.popular
                  ? "border-primary bg-linear-to-br from-primary/5 dark:from-primary/10 to-primary/10 dark:to-primary/20 shadow-lg shadow-primary/20 dark:shadow-primary/30 hover:shadow-xl hover:shadow-primary/30 dark:hover:shadow-primary/40"
                  : "border-border dark:border-gray-700 bg-background-secondary dark:bg-gray-800/50 hover:border-primary dark:hover:border-primary hover:bg-linear-to-br hover:from-primary/5 dark:hover:from-primary/10 hover:to-primary/10 dark:hover:to-primary/20 hover:shadow-lg hover:shadow-primary/20 dark:hover:shadow-primary/30"
              }
          ...
          >
            ...
            {pkg.popular && (
              <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-20">
                <span className="bg-linear-to-r from-primary to-secondary text-white text-xs font-bold px-3 py-1 rounded-full shadow-md dark:shadow-lg flex items-center gap-1">
            ...
            <div className="w-full flex flex-col items-center space-y-4">
              ...
              <div className="text-center">
                <div className="text-4xl font-bold bg-linear-to-r from-primary to-secondary bg-clip-text text-transparent dark:from-primary dark:to-secondary">
                  {pkg.credits}
                </div>
      ...
      <div className="mt-8 p-4 rounded-xl bg-success/10 dark:bg-success/20 border border-success/20 dark:border-success/30 flex items-start gap-3">
        <Shield className="w-5 h-5 text-success dark:text-success/90 shrink-0 mt-0.5" />
