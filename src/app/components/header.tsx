...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const handleRouteChange = () => {
      ...
      const isProjectDetailPage =
        newPath.startsWith("/projects/") && newPath !== "/projects";
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-16 left-0 right-0 z-40 bg-white border-b border-border shadow-lg transition-all duration-300 ${
          showHeaderSearch ? "translate-y-0" : "-translate-y-full"
        }`}
      >
                      ...
                      <nav className="flex-1 overflow-y-auto px-4 py-6 space-y-2">
                        {user ? (
                          ...
                          <>
                            ...
                            <Link
                              href="/auth/login"
                              className="block px-2 py-2 rounded hover:bg-background-secondary text-primary font-medium"
                              onClick={() => setIsDrawerOpen(false)}
                            >
                              {t("header.signIn")}
                            </Link>
                            <Link
                              href="/auth/signup"
                              className="block px-2 py-2 rounded hover:bg-primary text-white font-medium bg-primary mt-2 text-center"
                              onClick={() => setIsDrawerOpen(false)}
                            >
                              {t("header.signUp")}
                            </Link>
