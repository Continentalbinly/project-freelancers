...
export default function RecentProjects({ user }: any) {
  ...
  // 🎨 Status colors — includes in_review
  ...
  // 🏷️ Status labels — using common.status
  const getStatusLabel = (status: string) => {
    switch (status) {
      case "open":
        return t("common.status.statusOpen");
      case "in_progress":
        return t("common.status.statusInProgress");
      case "in_review":
        return t("common.status.statusInReview");
      case "completed":
        return t("common.status.statusCompleted");
      case "cancelled":
        return t("common.status.statusCancelled");
      default:
                ...
                {project.clientProfile && (
                  <div className="mt-auto flex items-center gap-2 pt-2 border-t border-border/60">
                    ...
                    <p className="text-xs text-text-secondary truncate">
                      ...
                      <span>
                        {project.clientProfile.rating
                          ? ` • ⭐ ${project.clientProfile.rating.toFixed(1)}`
                          : ` • ${t("common.noRating")}`}
                      </span>
                    </p>
                  </div>
